...
namespace DshNotifyicon.Services
{
    ...
    public static class EnvironmentCheckService
    {
        public static async Task<List<EnvItem>> CheckAllAsync(Settings settings, string envPath, Action<string> log = null)
        {
            ...
            // Node.js
            var node = await NodeService.DetectAsync(settings.NodePath);
            ...
            // npm 镜像源
            string reg = "";
            try { reg = (await NpmService.GetRegistryAsync(settings.MirrorUrl, envPath)).Trim(); }
            catch (Exception ex) { reg = "读取失败: " + ex.Message; }
            items.Add(new EnvItem
            {
                Name = "npm 镜像源",
                Status = reg.StartsWith("读取失败") ? EnvStatus.Error : EnvStatus.Ok,
                Detail = "npm 全局配置: " + reg +
            ...
            // dsh
            string local = "";
            try { local = await NpmService.GetDshLocalVersionAsync(envPath); } catch { }
            ...
            {
                string latest = null;
                try { latest = (await NpmService.GetDshLatestVersionAsync(settings.MirrorUrl, envPath)).Trim(); }
                catch (Exception ex) { latest = null; log?.Invoke("dsh 远端版本查询失败: " + ex.Message); }
